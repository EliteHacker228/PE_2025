...
def test_concurrent_processing(tmp_path_factory):
    """Тестирование обработки 10 изображений параллельно"""
    temp_dir = tmp_path_factory.mktemp("load_test")

    for i in range(10):
        img_path = temp_dir / f"test_{i}.jpg"
        cv2.imwrite(str(img_path), np.random.randint(0, 255, (800, 600, 3), dtype=np.uint8))

    with ThreadPoolExecutor(max_workers=5) as executor:
        results = list(executor.map(
            lambda p: process_image_file(str(p)),
            temp_dir.glob("*.jpg")
        ))
